...
//Crear descargas electricas

var origen = new Descarga(19.43,-99.13,320.5);
var rama1 = new Descarga(25.12, -72.62, 285.6);
var rama2 = new Descarga(19.33, -99.18, 210.7);
var rama2a = new Descarga(19.15, -99.17, 25.3);
...
//Registrar sensores en el diccionario
...
catch(SensorNoEncontradoExcepcion ex)
{
...
{
    public double Distancia(Descarga otra)
    {
        return Math.Sqrt(Math.Pow(Latitud - otra.Latitud,2) + Math.Pow(Longitud - otra.Longitud,0));
    }
...
public class ArbolRayo
{
    ...
    // DSF suma kV de todos los nodos del arbol
    ...
    //Imprime el arbol con sangria por cada nivel
...
public class SensorNoEncontradoExcepcion :  Exception
{
    public SensorNoEncontradoExcepcion(string id) : base($"Sensor {id} no registrado en la red")
    { }
}
...
public class SensorAcustico : SensorMeteorologico
{
    ...
    public override string medir()
    {
        return $"[CE-{Id}] Nivel sonoro: {new Random().Next(80, 130)} dB";
    }
}
...
public class RedSensores
{
    ...
    public SensorMeteorologico ObtenerPorID(string id)
    {
        ...
        {
            throw new SensorNoEncontradoExcepcion(id);
        }
        return sensor;
    }
...
public class AnalizadorTormenta : IAnalizador
{
    ...
    //Buscar en el diccionario el sensor mas cercano
    ...
    public void GenerarReporte()
    {
        ...
        Console.WriteLine($"[Intensidad acomulada] {IntensidadTotal()}");
